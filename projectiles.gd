...
export(String) var startpath  = "Start";
func _process(delta):
	var divisor = Vector2(1024,600);
	var pos = get_parent().get_parent().get_node(startpath).position/divisor;
	...
	position = get_parent().get_parent().get_node(startpath).position# + Vector2(0,-300)
